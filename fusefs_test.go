...
var _ = Describe("fusefs", func() {
	It("can be mounted", func() {
		...
		process, err := container.Run(garden.ProcessSpec{Path: "mkdir", Args: []string{"-p", mountpoint}}, silentProcessIO)
		Ω(err).ShouldNot(HaveOccurred())
		Ω(process.Wait()).Should(Equal(0), "Could not make temporary directory!")

		process, err = container.Run(garden.ProcessSpec{Privileged: true, Path: "/usr/bin/hellofs", Args: []string{mountpoint}}, silentProcessIO)
		Ω(err).ShouldNot(HaveOccurred())
		...
		process, err = container.Run(garden.ProcessSpec{Privileged: true, Path: "fusermount", Args: []string{"-u", mountpoint}}, silentProcessIO)
		Ω(err).ShouldNot(HaveOccurred())
